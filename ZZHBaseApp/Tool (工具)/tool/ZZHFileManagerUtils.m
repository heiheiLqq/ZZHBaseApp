...
#import "ZZHFileManagerUtils.h"

@implementation ZZHFileManagerUtils
/**
 *  创建一个单例
 *
...
+ (instancetype)sharedInstance {
    static ZZHFileManagerUtils *singleton = nil;
    static dispatch_once_t pred;
    dispatch_once(&pred, ^{
        singleton = [[ZZHFileManagerUtils alloc] init];
    });
    return singleton;
}
...
+ (void)createResDirByDirectoryName:(NSString *)directoryName {
    NSFileManager *fileManager = [NSFileManager defaultManager];
    BOOL isDir = TRUE;
    BOOL isDirExist = [fileManager fileExistsAtPath:[ZZHFileManagerUtils getOtherPath:directoryName] isDirectory:&isDir];
    if (!(isDirExist && isDir)) {
        BOOL dCreateDir = [fileManager createDirectoryAtPath:[[ZZHFileManagerUtils getDocumentsPath] stringByAppendingPathComponent:directoryName] withIntermediateDirectories:YES attributes:nil error:nil];
        if (!dCreateDir) {
            NSLog(@"Create Res Directory Failed.");
        }
    }
}
...
+ (void)renameFileByDirectoryName:(NSString *)directoryName oldFileName:(NSString *)oldFileName newFileName:(NSString *)newFileName {
    ...
    //显示文件目录的内容
    NSLog(@"Documentsdirectory: %@", [fileManager contentsOfDirectoryAtPath:[[self getDocumentsPath] stringByAppendingPathComponent:directoryName] error:&error]);
}
